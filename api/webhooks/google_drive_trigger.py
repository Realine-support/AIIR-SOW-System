...
async def google_drive_file_added(request: Request):
    ...
    try:
        ...
        # Get the generated document URLs from Tracker
        # The workflow updates the Tracker sheet with document URLs
        drive = GoogleDriveService(config.google_credentials_path)
        sheets = GoogleSheetsService(config.google_credentials_path)

        # Find the row in Tracker with this engagement_id
        tracker_data = sheets.read_sheet(
            config.tracker_sheet_id,
            config.tracker_tab_name,
            range_name=f"{config.tracker_tab_name}!A:Z"
        )

        # Find engagement row and extract URLs
        sow_url = None
        calculator_url = None

        if tracker_data and len(tracker_data) > 1:
            # Find header row to get column indices
            headers = tracker_data[0]
            engagement_col = headers.index("Engagement ID") if "Engagement ID" in headers else 0
            sow_col = headers.index("SOW Document URL") if "SOW Document URL" in headers else None
            calc_col = headers.index("Pricing Calculator URL") if "Pricing Calculator URL" in headers else None

            # Find the engagement row
            for row in tracker_data[1:]:
                if len(row) > engagement_col and row[engagement_col] == engagement_id:
                    if sow_col and len(row) > sow_col:
                        sow_url = row[sow_col]
                    if calc_col and len(row) > calc_col:
                        calculator_url = row[calc_col]
                    break
        ...
        return {
            ...
            "file_urls": {
                "sow_template": sow_url or "Processing - check Tracker sheet",
                "pricing_calculator": calculator_url or "Processing - check Tracker sheet"
            },
            "file_info": {
                "file_id": file_id,
                "filename": filename,
            ...
            "message": f"File '{filename}' processed successfully. Engagement ID: {engagement_id}",
            "tracker_sheet_url": f"https://docs.google.com/spreadsheets/d/{config.tracker_sheet_id}"
        }
